...
\oleks{Consider if we need to mention counter in Section 4.5}
\begin{verbatim}
    ...
    Output:
        ...
        c: final counter value used

    1. setTypeAndClear(ADRS, H_MSG)
    2. for c from 0 to 2^32 - 1:
        a. digest ← H_msg(ADRS, R, c, PK.seed, PK.root, m)
...
\end{verbatim}
\oleks{add expected counter value for different approaches}
...
\begin{verbatim}
    Algorithm: wots_digest(m, R, counter, PK.seed, PK.root, ADRS)
    Input:
        m: message
        R: randomness from signature
        counter: counter from signature
        PK.seed: public seed
        PK.root: public key root
        ADRS: tweak
    Output:
    ...
    1. setTypeAndClear(ADRS, H_MSG)
    2. digest ← H_msg(ADRS, R, counter, PK.seed, PK.root, m)
...
\begin{verbatim}
    ...
    Input:
        ...
        keypair: keypair index for this WOTS+ instance
    Output:
        sig: WOTS+C signature (R || counter || chain_values)

    1. opt_rand ← random(n) or PK.seed  // for randomized or deterministic signing
    2. R ← PRF_msg(SK.prf, PK.seed, opt_rand, m)
    3. (msg, counter) ← wots_grind(m, R, PK.seed, PK.root, ADRS)
    4. sig ← R || toByte(counter, c)
    5. for i from 0 to len - 1:
        a. setTypeAndClear(ADRS, PRF)
        b. setKeyPairAddress(ADRS, keypair)
        c. setChainAddress(ADRS, i)
