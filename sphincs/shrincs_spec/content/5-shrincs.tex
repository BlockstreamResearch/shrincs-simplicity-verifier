...
In this topology the \texttt{q}-th signature requires a path of length \texttt{q+1}. This linear growth is superior to logarithmic growth for small \texttt{q}. For \texttt{q=0}, the path is just 16 bytes. Even for \texttt{q=10}, the path is $11 \times 16 = 176$ bytes, which is extremely small compared to the fixed $\approx 2.5$KB of a balanced tree or the $\approx 8$KB of SLH-DSA.
...
\begin{itemize}
    \item Level 0 (top): WOTS+C public key for signature \texttt{q=1}
    ...
    \begin{itemize}
        ...
        \item Left child: WOTS+C public key for signature \texttt{q=hsf+1}
    \end{itemize}
\end{itemize}
...
\begin{verbatim}
    ...
    Input:
        ...
        auth: authentication path (array of q nodes)
        q: signature number
        PK.seed: public seed
        ADRS: tweak
    Output:
    ...
    3. if q <= h_sf:
        for i from 0 to q - 1:
            ...
            e. else:
                node ← Tw(PK.seed, ADRS, node || auth[q-1])
    4. else: 
        a. setTreeHeight(ADRS, h_sf)
        b. setTreeIndex(ADRS, 0)
        c. node ← Tw(PK.seed, ADRS, auth[h_sf-1] || node)
        d. for i from 0 to h_sf - 2:
            level ← h_sf - 1 - i
            setTreeHeight(ADRS, level)
            setTreeIndex(ADRS, 0)
            if i < h_sf - 2:
                node ← Tw(PK.seed, ADRS, auth[i] || node)
            else:
                node ← Tw(PK.seed, ADRS, node || auth[h_sf-2])
    5. return node
\end{verbatim}
