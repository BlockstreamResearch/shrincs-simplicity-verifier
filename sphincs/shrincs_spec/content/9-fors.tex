...
\begin{figure}[h!]
    ...
    \label{fig:shrincs}
\end{figure}
The FORS public key is the tweakable hash of the tree roots. With the +C grinding optimization, the last tree's root is excluded: \texttt{FORS+C PK = Tw(PK.seed, ADRS, root\_0 || ... || root\_{k-2})}, using address type \texttt{FORS\_PK}. This is valid because grinding guarantees \texttt{indices[k-1] == 0}, so the verifier can independently recompute \texttt{root\_{k-1}} if needed (though it is not used in the public key computation).
...
Expected grinding cost is $\approx 2^a$ iterations. With a = 22, this is approximately 4 million iterations. Since \texttt{H\_msg\_fors} operates on fixed-size inputs, each iteration is a single SHA-256 call.
...
\begin{verbatim}
    ...
    1. setTypeAndClear(ADRS, FORS_GRIND)
...
\begin{verbatim}
    ...
    3. right ← fors_treehash(SK.seed, PK.seed, ADRS, tree_idx, target_height - 1, start_idx + 2^(target_height - 1))
...
The \texttt{fors\_pkFromSig} function recovers the FORS+C public key from a signature. For each of the first k-1 trees, it hashes the revealed secret key to obtain the leaf, then iteratively combines with authentication path nodes to compute the tree root. The FORS+C public key is the tweakable hash of these \texttt{k-1} roots (tree \texttt{k-1} is excluded entirely due to grinding). The function returns $\bot$ if the grinding condition is not satisfied.
\begin{verbatim}
    ...
    Input:
        ...
        ADRS: tweak
    Output:
        pk: FORS+C public key (n bytes), or $\bot$ if invalid

    1. R ← sig[0 : R_SIZE]
    2. ctr ← sig[R_SIZE : R_SIZE + c]
    3. setTypeAndClear(ADRS, FORS_GRIND)
    4. digest ← H_msg_fors(ADRS, R, toByte(ctr, c), PK.seed, PK.root, m)
    ...
    9. for i from 0 to k - 2:
        ...
        j. for h from 0 to a - 1:
            ...
            idx ← idx >> 1
        k. roots[i] ← node
    10. setTypeAndClear(ADRS, FORS_PK)
    11. pk ← Tw(PK.seed, ADRS, roots[0] || ... || roots[k-2])
    12. return pk
\end{verbatim}