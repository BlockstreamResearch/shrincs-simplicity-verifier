...
Converts message digest into k indices, each selecting a leaf in the corresponding FORS tree.
\begin{verbatim}
...
\subsection{Grinding}
Finds counter value where the last tree's index is 0, eliminating need to send its authentication path.
\begin{verbatim}
...
Derives a secret leaf value for a specific tree and leaf index using PRF.
\begin{verbatim}
...
\subsection{TreeHash}
Recursively computes FORS subtree roots, used for both root computation and authentication path generation.
\begin{verbatim}
...
Generates the a sibling nodes needed to authenticate a revealed leaf up to the tree root.
\begin{verbatim}
...
Signs by revealing k-1 leaves with full auth paths, plus a special case for tree k-1 where grinding ensures index=0
\begin{verbatim}
...
Recovers FORS public key from signature by recomputing tree roots.
\begin{verbatim}
