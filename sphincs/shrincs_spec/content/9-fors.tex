...
\begin{itemize}
    \item Each tree has \texttt{pow(2,a)} leaves
...
The \texttt{fors\_msg\_to\_indices} function converts a message digest into k leaf indices, one for each FORS tree. Each index is extracted as a consecutive a-bit chunk from the digest, selecting which leaf to reveal from the corresponding tree. The indices are integers in the range \texttt{[0, pow(2,a) - 1]}.
\begin{verbatim}
