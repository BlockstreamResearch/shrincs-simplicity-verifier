...
%\mknote{I meean, it is possible to understand, but I think it is better to unify the numbering.}
The UXMSS tree is constructed recursively as a right-skewed binary tree with \texttt{h\_sf + 1} leaves. To describe the tree structure, we use "level" to denote depth from the root, with level 0 at the root and level \texttt{h\_sf - 1} at the bottom internal node. Note that this UXMSS convention is the inverse of the convention used in the balanced XMSS trees of the stateless component (Section~7). However, the ADRS height field for internal nodes is set to (\texttt{h\_sf - level}), so both constructions use increasing height values toward the root.
The UXMSS tree is constructed as follows:
\begin{itemize}
    \item Level 0 is the root public key \texttt{PK.sf}.
    \item Level \texttt{i} (for \texttt{0 <= i <= h\_sf-2}, internal):
    \begin{itemize}
...
This means the tree has \texttt{h\_sf + 1} leaves (one per valid signature index \texttt{q $\in$ \{1, ..., h\_sf + 1\}}), and every internal node has a WOTS+C leaf as its left child and a UXMSS subtree (or a second leaf at the bottom) as its right child.
...
\begin{verbatim}
    ...
    Input:
        ...
        level: depth from root (0 = root, h_sf - 1 = bottom internal node)
    Output:
        node: hash value (n bytes)

    1. left ← wots_PKgen(SK.seed, PK.seed, ADRS, level + 1, SF)
    2. if level == h_sf - 1:
            right ← wots_PKgen(SK.seed, PK.seed, ADRS, h_sf + 1, SF)
        else:
            right ← uxmss_treehash(SK.seed, PK.seed, ADRS, level + 1)
    3. setTypeAndClear(ADRS, SF_TREE)
    4. setTreeHeight(ADRS, h_sf - level)
    5. setTreeIndex(ADRS, 0)
    6. return Tw(PK.seed, ADRS, left || right)
\end{verbatim}
\paragraph{Note:} The keypair index passed to \texttt{wots\_PKgen} corresponds to the signature number \texttt{q}. At level \texttt{i}, the left child is the leaf for \texttt{q = i + 1}. At the bottom level (\texttt{h\_sf - 1}), the right child is the leaf for \texttt{q = h\_sf + 1}.
...
\mknote{I think we still can use idx variable, and just use it as a pointer to the leaf, counted from left to right.}
\oleks{we can, but I think the semantics of \texttt{q} and \texttt{idx} are different. \texttt{idx} is a position in a symmetric tree where all leaves are equivalent, the auth path is always \texttt{h'} nodes regardless of which \texttt{idx} we pick. \texttt{q} is the signature counter that directly determines the auth path length and structure}
...
\begin{verbatim}
    ...
    6. return auth
\end{verbatim}
\paragraph{Note:} For \texttt{q <= h\_sf}, the authentication path has exactly q nodes. For \texttt{q = h\_sf + 1}, it has \texttt{h\_sf} nodes, since the rightmost leaf shares its parent's other child (a WOTS+C key) rather than a subtree root.
...
\begin{itemize}
    \item Case \texttt{q <= h\_sf}: The recovered key is a left child. The first combination pairs the recovered key (left) with the first auth node (right). All subsequent combinations have the auth node (a WOTS+C key from a lower \texttt{q} value) on the left and the current node on the right.
    \item Case \texttt{q = h\_sf + 1}: The recovered key is the rightmost leaf (a right child). All auth nodes are WOTS+C keys on the left.
\end{itemize}


%\mknote{The checks at 7.c can be just moved outside the loop. First sibling is always left, then it is always right. }
\begin{verbatim}
    ...
    Input:
        ...
        q: signature number
    Output:
        root: computed PK.sf, or $\bot$ if invalid
    ...
    7. if q <= h_sf:
        a. setTreeHeight(ADRS, h_sf - (q - 1))
        b. setTreeIndex(ADRS, 0)
        c. node ← Tw(PK.seed, ADRS, node || auth[0 : n])
        for i from 1 to q - 1:
            a. setTreeHeight(ADRS, h_sf - (q - 1 - i))
            b. setTreeIndex(ADRS, 0)
...
The resulting signature size is \texttt{|wots\_sig| + q*n} bytes, which grows linearly with \texttt{q}. For the first signature (q = 1), this yields the minimal signature that makes SHRINCS attractive for Bitcoin applications.
\begin{verbatim}
...
\begin{verbatim}
    SHRINCS-B:
        ...
        q_max=226:  32 + 4 + 16*16 + 16*226 =   3908 bytes // < 3992 bytes (stateless)
    SHRINCS-L:
        q=1:        32 + 4 + 64*16 + 16 =       1076 bytes
        q=10:       32 + 4 + 64*16 + 16*10 =    1220 bytes  
        q_max=418:  32 + 4 + 64*16 + 16*418 =   7748 bytes // < 7832 bytes (stateless)
\end{verbatim}

Verification cost per UXMSS signature is calculated as WOTS+C verification cost + \texttt{q} hashes.
