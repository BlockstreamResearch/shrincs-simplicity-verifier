...
The stateless signature mode uses a SPHINCS+-style hypertree consisting of \texttt{d} layers, where each layer contains standard balanced XMSS trees. Each XMSS tree authenticates \texttt{2\string^h'} WOTS+C key pairs, where \texttt{h' = h\_sl / d} is the height of trees within each layer.
...
For a leaf at index \texttt{idx}, the authentication path consists of the sibling nodes at each level from the leaf up to (but not including) the root. The authentication path contains \texttt{h'} nodes, each of size \texttt{n} bytes, allowing a verifier to reconstruct the path from the leaf to the root.
...
Each XMSS signature consists of a WOTS+C signature plus an authentication path of \texttt{h'} nodes: \texttt{R\_SIZE + c + (l + h') * n} bytes (420 bytes for SHRINCS-B and 1188 bytes for SHRINCS-L).

The full stateless hypertree consists of d = 5 layers, giving:
\begin{itemize}
    \item 2100 bytes and 10245 hashes for SHRINCS-B
    \item 5940 bytes and 305 hashes for SHRINCS-L
\end{itemize}
