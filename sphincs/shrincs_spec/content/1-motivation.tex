...
\begin{enumerate}
    \item Signature size: NIST-standardized SLH-DSA signatures \cite{slhdsa_fips205} range from 7KB to 50KB, significantly larger than current 64-byte Schnorr signatures. This increases transaction fees and reduces the network's throughput.
...
\subsection{SHRINCS} 
SHRINCS (Stateful Hash-based Robust Integrated NIST-Compatible Signatures) is a hybrid post-quantum signature scheme that combines the efficiency of stateful signatures with the robustness of stateless ones. It provides with two parameters set depending on what matters for verifier:
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{Efficient stateful path.} During normal operation with intact state, SHRINCS uses an Unbalanced XMSS (UXMSS) tree with WOTS+C one-time signatures. Signature sizes are:
    \begin{itemize}
        \item SHRINCS-B: \texttt{308 + 16q}  
        \item SHRINCS-L: \texttt{1076 + 16q} 
    \end{itemize} 
    bytes (where \texttt{q >= 1}) is the signature counter. So the first SHRINCS-B signature is $\times 20$ smaller than the smallest SLH-DSA variant.
    \item \textbf{Robust stateless fallback.} When the state is lost, corrupted, or exhausted, SHRINCS falls back to a SPHINCS+-based stateless signature (4008 or 7848 bytes). This is still smaller than standard SLH-DSA.
