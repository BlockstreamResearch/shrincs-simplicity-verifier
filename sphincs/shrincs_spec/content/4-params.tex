...
\begin{tabular}{ c c c }
 ...
 \texttt{a} & $\color{red}14\color{black}$ & FORS+C tree height \\
 \texttt{k} & $\color{red}11\color{black}$ & Number of FORS+C trees \\
...
\end{verbatim}
where \texttt{ADRS} is a context address (see Section 4.1). \\
...
To prevent multi-target attacks and ensure domain separation between the various components, we employ the tweakable hash function construction defined in FIPS 205:
...
\begin{center}
    \texttt{PRF(PK.seed, ADRS, SK.seed) = Tw(PK.seed || ADRS || SK.seed)[0:n]}
\end{center}
...
\begin{center}
    \texttt{PRF\_msg(SK.prf, PK.seed, opt\_rand, m) = HMAC-SHA-256(SK.prf, PK.seed || opt\_rand || m)[0:n]},
\end{center}
