...
Additionally we can manipulate a target condition to decrease the cost of signature verification. The signature generation and verification operations together consist of \texttt{St = (w-1)*l} hashing operations. Increasing \texttt{S\_wn} increases grinding cost but reduces the verification complexity to \texttt{(S\_t - S\_wn)} hashes. Concrete parameters are presented in Section~4.

%\mknote{Plus grinding, Plus public key compression but we can also put that cost to the tree generation part. Moreover, during signing in the XMSS we need to compute all the chains in the WOTS anyways, since we need to get to public key to compute the leaf of the Merkle tree.}

\subsection{Base-w Representation}
WOTS represents a message digest in base \texttt{w} so that each digit is an integer in \texttt{[0...w-1]}. These digits directly determine the number of hashes to apply to each WOTS chain element during signing and verification. The \texttt{base\_w()} function converts a byte string into an array of \texttt{out\_len} base-w digits. In this specification we assume w is a power of two for efficiency, though this is not a strict requirement of the construction.
...
The \texttt{mode} parameter determines which address types are used: \texttt{SF} for the stateful tree instance \texttt{(SF\_WOTS\_HASH, SF\_WOTS\_PK, SF\_TREE)} or \texttt{SL} for the stateless instance \texttt{(SL\_WOTS\_HASH, SL\_WOTS\_PK, SL\_TREE)}.
\begin{verbatim}
...
\begin{verbatim}
    ...
    2. for i from 0 to l - 1:
        a. setTypeAndClear(ADRS, WOTS_PRF)
...
\begin{enumerate}
    \item Phase 1 \texttt{(H\_msg)}: Computes a randomized digest of the user message. This is performed once per signature and produces an n-byte digest. For internal XMSS signatures (which sign tree roots that are already hashes), Phase 1 is skipped and the tree root is used directly as the digest.
    \item Phase 2 \texttt{(H\_grind)}: Takes the Phase 1 digest (or tree root) and a counter, producing a candidate n-byte output. This is iterated during grinding until the target condition is met.
\end{enumerate}
...
\begin{verbatim}
    Algorithm: wots_grind(digest, ADRS)
    Input:
    ...
    1. setTypeAndClear(ADRS, WOTS_GRIND)
    2. for ctr from 0 to 2^32 - 1:
        a. grind_out ← H_grind(ADRS, digest, toByte(ctr, c))
        ...
        e. if sum == S_wn:
            return (msg, ctr)
    3. abort with error  // caller should resample R and retry
\end{verbatim}
...
\begin{verbatim}
    Algorithm: wots_digest(digest, ctr, ADRS)
    Input:
    ...
    1. setTypeAndClear(ADRS, WOTS_GRIND)
    2. grind_out ← H_grind(ADRS, digest, toByte(ctr, c))
    3. msg ← base_w(grind_out, w, l)
    4. sum ← 0
...
\begin{verbatim}
    ...
    4. (msg, ctr) ← wots_grind(digest, ADRS)
    5. sig ← R || toByte(ctr, c)
...
\begin{verbatim}
    ...
    4. if is_internal:
            ...
            setTypeAndClear(ADRS, H_MSG)
            digest ← H_msg(ADRS, R, PK.seed, PK.root, m)
    5. (msg, valid) ← wots_digest(digest, ctr, ADRS)
    6. if not valid:
        return $\bot$
    7. tmp ← []
