...
The general algorithm for base-w representation is following:
...
But we provide optimized versions for \texttt{w = 256, 4} proposed by this specification. For \texttt{w=256}:
\begin{verbatim}
...
\begin{verbatim}
    ...
    Input:
        m: byte string
        out_len: number of base-4 digits
    Output:
...
To create a WOTS public key for a given \texttt{keypair} index, we derive one secret starting value per chain using \texttt{PRF}, then advance each value to the end of its chain with \texttt{chain(..., w-1)}. The resulting array of end-of-chain values is compressed to a fixed-size WOTS public key for that \texttt{keypair}.
\begin{verbatim}
