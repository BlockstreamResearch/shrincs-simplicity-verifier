...
\begin{enumerate}
    ...
    \item Phase 2 \texttt{(H\_grind)}: Takes the phase 1 digest (or tree root) and a counter, producing a candidate \texttt{n}-byte output. This is iterated during grinding until the target condition is met.
\end{enumerate}
...
\begin{verbatim}
    ...
    2. for ctr from 0 to 2^32 - 1:  \\ c = 4 bytes
        a. grind_out ← H_grind(ADRS, digest, ctr, c)
...
\end{verbatim}
\oknote{to add expected grinding complexity for different approaches}
...
\begin{verbatim}
    ...
    1. setTypeAndClear(ADRS, WOTS_GRIND)
    2. grind_out ← H_grind(ADRS, digest, ctr)
