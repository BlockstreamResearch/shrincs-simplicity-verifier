...
This checksum is then signed using additional chains. In WOTS+C\cite{wots+c}, instead of calculating a checksum  for a fixed message digest, the signer modifies the message digest until it possesses a specific,  pre-determined checksum property. This is done by appending a counter \texttt{ctr} and a random salt \texttt{R} \mknote{I think we should separate those, first we use Hmsg to hash the message with R, and they we use that digest and hash it again with the counter until we find a good digest. I can try to investigate if it is possible to combine those, but it looks like two separate hashes is better.} to the message before hashing. The signer increments \texttt{ctr} until the resulting hash digest $D$ satisfies a global target condition.
...
Additionally we can manipulate a target condition to decrease the cost of signature verification. The signature generation and verification operations together consist of \texttt{St = (w-1)*l} hashing operations \mknote{Plus grinding, Plus public key compression but we can also put that cost to the tree generation part. Moreover, during signing in the XMSS we need to compute all the chains in the WOTS anyways, since we need to get to public key to compute the leaf of the Merkle tree.}. So we can increase \texttt{Swn} (grinding complexity) and receive the reduced verification complexity of \texttt{(St - Swn)} hashes. Concrete parameters are presented in Section~4.

\subsection{Base-w Representation}
WOTS represents a message digest in base \texttt{w} so that each digit is an integer in \texttt{[0...w-1]}. These digits directly determine the number of hashes to apply to each WOTS chain element during signing and verification. The \texttt{base\_w()} function converts a byte string into an array of \texttt{out\_len} base-w digits (this requires \texttt{w} to be a power of two.)\mknote{It does not have to be a power of two, but it is more convinient, if it is. So yes, in this document we can assume for it to be a power of 2, but I would change the wording since it is not a strict requirement}.
\begin{verbatim}
...
\begin{verbatim}
    ...
    6. return pk
\end{verbatim}
\mknote{Was the addrs for WOTS compression defined before? The computation of pk is never explained I think}

\subsection{Messsage Digest Grinding}
\mknote{Here we need to split message hashing into two parts. First is message digest computation with Randomness. Next hashing of the digest with the counter. I think this is better for security proofs and presents a better modular design. Because in stateless branch you will also hash for WOTS+C where you will not use any randomness, just the previous hash and the counter. Important remark for future: message hashing for SPHINCS+C  can differ in output length from the hashing for XMSS. But if we dont put the counter in neither cases we will have the same inouts.}
...
\texttt{R} and grinding counter \texttt{ctr} needed for the verifier to reproduce the same message digits.\mknote{Also split the message hashing here.}
\begin{verbatim}
...
In this section we have defined basic WOTS+C operations for stateful tree instance. But actually the same one time signature is used in stateless instance too. So we recommend to have a \texttt{mode} parameter that defines the \texttt{ADRS} with \texttt{SF\_WOTS\_CHAIN / SL\_WOTS\_CHAIN} and \texttt{SF\_TREE / SL\_TREE} correspondingly.