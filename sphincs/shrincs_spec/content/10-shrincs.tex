...
The \texttt{SHRINCS.SignStateful} function signs a message using the efficient stateful (UXMSS) path. It first checks that state is valid and that signature slots remain (\texttt{q <= h\_sf + 1}). It then increments \texttt{q} and updates the state before generating the signature. This ordering is critical: if the process is interrupted after signing but before state persistence, the one-time key would be consumed without the state reflecting it, risking catastrophic key reuse on retry (see Section~1.2).

The signature includes \texttt{PK.sl} prepended to enable unified verification. This produces minimal signatures (324 bytes for q = 1 in SHRINCS-B) that grow linearly with \texttt{q}. If state is invalid or exhausted, the function returns $\bot$ and the caller should use stateless signing instead.
\begin{verbatim}
...
The digest output length is \texttt{(roundup(k*a + h\_sl) / 8)} bytes. The first \texttt{k*a} bits encode the \texttt{k} FORS leaf indices (with the last a bits required to be zero by grinding). The remaining \texttt{h\_sl} bits encode the hypertree path indices.
...
\begin{verbatim}
    ...
    4. setTypeAndClear(ADRS, FORS_GRIND)
    5. for ctr from 0 to 2^32 - 1:
        a. digest ← H_msg_fors(ADRS, R, ctr, PK.seed, PK.root, m)
        b. indices ← fors_msg_to_indices(digest)
        c. if indices[k-1] == 0:
            break
    6. if ctr == 2^32:
        abort with error
    7. (tree_idx, leaf_idx) ← parse_idx(digest)
    8. setLayerAddress(ADRS, 0)
    9. setTreeAddress(ADRS, tree_idx[0])
    10. fors_sig ← R || ctr
    11. for i from 0 to k - 2:
        a. sk_i ← fors_SKgen(SK.seed, PK.seed, ADRS, i, indices[i])
        b. auth_i ← fors_auth_path(SK.seed, PK.seed, ADRS, i, indices[i])
        c. fors_sig ← fors_sig || sk_i || auth_i
    12. fors_pk ← fors_pkFromSig(fors_sig, m, PK.seed, PK.root, ADRS)
    13. ht_sig ← []
    14. msg ← fors_pk
    15. for layer from 0 to d - 1:
    ...
    16. sig ← PK.sf || fors_sig || ht_sig
    17. return sig
\end{verbatim}

\subsection{Index Parsing}
The \texttt{parse\_idx} function extracts tree and leaf indices for each hypertree layer from the stateless digest. In the unified digest scheme, these bits follow the \texttt{k*a} bits used for FORS indices. The \texttt{h\_sl} bits are partitioned into \texttt{d} pairs of \texttt{(leaf\_idx, tree\_idx)}, extracted from least significant (layer 0) to most significant (layer \texttt{d-1}), with \texttt{h'} bits per leaf index.
...
\begin{verbatim}
    ...
    1. Extract hypertree bits starting after FORS indices
    2. ht_offset ← k * a
    3. ht_bits ← extract_bits(digest, ht_offset, h_sl)
    4. idx ← ht_bits 
    5. tree_idx ← []
    6. leaf_idx ← []
    7. for layer from 0 to d - 1:
        a. leaf_idx[layer] ← idx AND (2^h' - 1)
        b. idx ← idx >> h'
        c. tree_idx[layer] ← idx AND (2^(h_sl - (layer + 1) * h') - 1)
    8. return (tree_idx, leaf_idx)
\end{verbatim}
\paragraph{Note:} The total digest must encode \texttt{k*a + h\_sl = 6*22 + 40 = 172} bits. The \texttt{H\_msg\_fors} output length (Section~5.4.2) must be set accordingly: \texttt{roundup((k*a + h\_sl) / 8) = 22} bytes. This is a correction from the original \texttt{roundup((k*a) / 8) = 17} bytes, which was insufficient for the unified digest.
...
\begin{verbatim}
    ...
    9. setTypeAndClear(ADRS, FORS_GRIND)
    10. digest ← H_msg_fors(ADRS, R, ctr, PK.seed, PK.root, m)
    11. indices ← fors_msg_to_indices(digest)
    12. if indices[k-1] != 0:
        return false
    13. (tree_idx, leaf_idx) ← parse_idx(digest)
    14. setLayerAddress(ADRS, 0)
    15. setTreeAddress(ADRS, tree_idx[0])
    16. fors_pk ← fors_pkFromSig(fors_sig, m, PK.seed, PK.root, ADRS)
    17. if fors_pk == $\bot$:
        return false  
    18. msg ← fors_pk
    19. xmss_sig_size ← R_SIZE + c + l * n + h' * n
...
The \texttt{SHRINCS.Verify} function provides a single entry point for verifying either stateful or stateless signatures. It distinguishes between the two types based on signature length: stateful signatures have a maximum size of \texttt{max\_sf\_size = n + R\_SIZE + c + l*n + (h\_sf + 1)*n} bytes, while stateless signatures are strictly larger (see Section~8.7 for the derivation that this bound always holds).
\begin{verbatim}
    ...
    1. max_sf_size ← n + R_SIZE + c + l * n + (h_sf + 1) * n
