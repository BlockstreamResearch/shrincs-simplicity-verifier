...
The WOTS+C component is 292 bytes: randomness (32 bytes) + counter (4 bytes) + chain values (256 bytes). The authentication path for signature \texttt{q} (count after signing) is \texttt{q * n = q * 16} bytes. The stateless public key \texttt{PK.sl} adds 16 bytes.
\begin{center}
