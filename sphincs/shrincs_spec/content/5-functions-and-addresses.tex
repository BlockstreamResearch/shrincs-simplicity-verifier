...
\paragraph{Phase 1. Message Digest}
Computes a randomized hash of the user message:
\begin{center}
...
\paragraph{Phase 2. Grinding Hash}
Takes the digest and a counter:
\begin{center}
    \texttt{H\_grind(ADRS, digest, ctr) = SHA-256(ADRS || digest || ctr)[0:n]}
\end{center}
This is computed repeatedly during grinding, but operates only on small fixed-size inputs (52 bytes), making iterations fast.
...
\begin{center}
    \texttt{H\_msg\_fors(ADRS, R, ctr, PK.seed, PK.root, m) = SHA-256(ADRS || R || ctr || PK.seed || PK.root || m)[0:(k*a)/8]}
\end{center}
...
0x0C: PRF:          Pseudorandom function
0x0D: ROOT:         Root public key computation
\end{verbatim}
...
%\mknote{I think SLH-DSA has more ADRS types, such as public-key compression address, FORS tree roots compression address, and we also need need two types for grinding WOTS and for grinding FORS}
...
0x0C & \texttt{PRF}             & keypair/tree\_idx & chain/0 & 0/leaf\_idx \\
0x0D & \texttt{ROOT}            & 0         & 0       & 0 \\
\hline
\end{tabular}
\caption{Type-dependent interpretation of ADRS words \texttt{(bytes [20:32])}. For PRF \texttt{(0x0C)}, field interpretation depends on context: WOTS uses \texttt{(keypair, chain, 0)}, FORS uses \texttt{(tree\_idx, 0, leaf\_idx)}. Compression addresses \texttt{(SF\_WOTS\_PK, SL\_WOTS\_PK, FORS\_PK)} use only keypair/tree identification}
\end{table}
