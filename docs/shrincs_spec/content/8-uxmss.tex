...
\begin{verbatim}
    ...
    6. return auth
\end{verbatim}
\paragraph{Note:} For \texttt{q <= h\_sf}, the authentication path has exactly \texttt{q} nodes. For \texttt{q = h\_sf + 1}, it has \texttt{h\_sf} nodes, since the rightmost leaf shares its parent's other child (a WOTS+C key) rather than a subtree root.
...
\begin{verbatim}
    SHRINCS-B:
        ...
        q_max=159:  32 + 4 + 16*16 + 16*159 =   2836 bytes // < 2860 bytes (stateless)
    SHRINCS-L:
        q=1:        32 + 4 + 64*16 + 16 =       1076 bytes
        q=10:       32 + 4 + 64*16 + 16*10 =    1220 bytes  
        q_max=207:  32 + 4 + 64*16 + 16*207 =   4372 bytes // < 4396 bytes (stateless)
\end{verbatim}
