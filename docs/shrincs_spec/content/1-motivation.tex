...
\begin{itemize}
    ...
    \item \textbf{Robust stateless fallback.} When the state is lost, corrupted, or exhausted, SHRINCS falls back to a SPHINCS+-based stateless signature (2568 or 4104 bytes). This is still smaller than standard SLH-DSA.
    ...
    \item \textbf{The number of supported signatures}. The construction supports up to 141 (SHRINCS-B) and 189 (SHRINCS-L) stateful signatures and up to $2^{20}$ stateless signatures, which should be enough for wallets. 
\end{itemize}
