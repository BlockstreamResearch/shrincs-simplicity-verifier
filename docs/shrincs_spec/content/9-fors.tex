...
%\mknote{The verifier can not recompute the last tree. It is just not needed, because we always have to select the leaf 0. Hence, we dont need the last tree at all.}
...
%\mknote{If it had variable size inputs would it be multiple SHA calls?}
...
%\mknote{grinding should be done in PRF\_msg}
...
\begin{verbatim}
    ...
    3. (indices, ctr, digest) ← fors_grind(m, R, PK.seed, PK.root, ADRS)
    4. sig ← R || ctr
...
\begin{verbatim}
    ...
    4. digest ← H_msg_fors(ADRS, R, ctr, PK.seed, PK.root, m)
