...
\begin{verbatim}
    ...
    1. R ← sig[0 : R_SIZE]
    2. setTypeAndClear(ADRS, H_MSG)
    3. digest ← H_msg_fors(ADRS, R, PK.seed, PK.root, m)
    4. indices ← fors_msg_to_indices(digest)
    5. if indices[k-1] != 0:
        return false
    6. roots ← []
    7. offset ← R_SIZE
    8. for i from 0 to k - 2:
        ...
        j. for h from 0 to a - 1:
            ...
            idx ← idx >> 1
        k. roots[i] ← node
    9. setTypeAndClear(ADRS, FORS_PK)
    10. pk ← Tw(PK.seed, ADRS, roots[0] || ... || roots[k-2])
    11. return pk
\end{verbatim}