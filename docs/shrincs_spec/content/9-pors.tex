...
Let candidates will represent potencial index to be added in the indices array. In our case, for chosen parameters of SHRINCS-B and SHRINCS-L, we will need \texttt{C' := roundup(2\string^b * k / t) = roundup(2\string^24 * 6 / 9245141) = 11} expected candidates (Appendix~1.1) to obtain \texttt{k} distinct indicesin range $[0,t-1]$.
...
Overall, we set $448$ to use in rejection sampling and $22$ bits in hypertree, resulting in 470 bits or 59 bytes.
