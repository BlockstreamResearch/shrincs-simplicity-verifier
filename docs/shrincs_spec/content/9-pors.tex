...
The length of the XOF output is \texttt{total\_bits = (roundup(2\string^b * k / t) + margin) * b + offset + hsl} (see Section~5.4.2). 
In our case, for chosen parameters of SHRINCS-B and SHRINCS-L, and \texttt{margin = 7}, \texttt{offset = 16}, we get the follwing length: \texttt{total\_bits = (roundup(2\string^24 * 6 / 9245141)) * 24 + 16 + 24 = 472} bits or 59 bytes.
...
Verification succeeds if and only if the recovered root matches \texttt{PK.root}. We also check the correctness of padding in the signature.
The function returns \texttt{"false"} if the grinding condition is not satisfied (i.e., the Octopus authentication set exceeds \texttt{m\_max}) or if the padding values are not all $0^{n}$.
