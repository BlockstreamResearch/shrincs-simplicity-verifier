...
\begin{verbatim}
    Algorithm: wots_grind(PK.seed, digest, ADRS, keypair)
    Input:
        PK.seed: public seed
...
\begin{verbatim}
    Algorithm: wots_digest(PK.seed, digest, ctr, ADRS, keypair)
    Input:
...
\begin{verbatim}
    ...
    4. (msg, ctr) ← wots_grind(PK.seed, digest, ADRS, keypair)
    5. sig ← R || ctr
    6. for i from 0 to (l - 1):
        a. setTypeAndClear(ADRS, WOTS_PRF)
        b. setKeyPairAddress(ADRS, keypair)
...
\begin{verbatim}
    ...
    4. if is_internal:
            ...
            setTypeAndClear(ADRS, H_MSG)
            digest ← H_msg(ADRS, R, PK.seed, PK.root, m)
    5. (msg, valid) ← wots_digest(PK.seed, digest, ctr, ADRS, keypair)
    6. if not valid:
        return false
    7. tmp ← []
