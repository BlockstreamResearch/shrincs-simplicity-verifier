...
Let \texttt{C := roundup(k * 2\string^b / t) + 7}, so the XOF output length is \texttt{roundup((C * b + 16 + hsl)/8)}. The first \texttt{C * b + 16} bits encode the \texttt{k} PORS+FP leaf indices. The remaining \texttt{hsl} bits encode the hypertree path indices.
...
\end{verbatim}
\paragraph{Note:} The total XOF output must encode \texttt{C * b + hsl = 18 * 24 + 16 + 24 = 472} bits. 
...
\begin{verbatim}
    ...
    15. setTreeAddress(ADRS, tree_idx[0] * 2^h_prime + leaf_idx[0])
