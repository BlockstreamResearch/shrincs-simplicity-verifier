...
%\mknote{The algorithm says: compute a FORS signature for ADDR = 0, then choose  FORS instance based on the hash, then compute PK for the FORS at ADRS = 0. This definetly wrong.}
...
%\mknote{parse\_hypertree\_idx is undefined}
...
\begin{verbatim}
    ...
    5. (tree_idx, leaf_idx) ← parse_hypertree_idx(digest)
    6. fors_pk ← fors_pkFromSig(fors_sig, m, PK.seed, PK.root, ADRS)
    7. ht_sig ← []
    8. msg ← fors_pk
    9. for layer from 0 to d - 1:
...
\end{verbatim}
\paragraph{Note:} The total digest must encode \texttt{k*a + h\_sl = 6*22 + 24 = 156} bits. The \texttt{H\_msg\_fors} output length (Section~5.4.2) must be set accordingly: \texttt{roundup((k*a + h\_sl) / 8) = 20} bytes. This is a correction from the original \texttt{roundup((k*a) / 8) = 17} bytes, which was insufficient for the unified digest.
...
%\mknote{Verification does not take which FORS instance is used as well. Same problems as in signing part.}
\begin{verbatim}
...
%\mknote{Did you test that Verify(Sign(message))=true for both cases?}
