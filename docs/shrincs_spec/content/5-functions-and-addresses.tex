...
\begin{center}
    \texttt{H\_msg\_fors(ADRS, R, PK.seed, PK.root, m) = SHA-256(ADRS || R || PK.seed || PK.root || m)[0:roundup((k*a + h\_sl) / 8)]}
\end{center}

The output length is roundup((k*a + h\_sl) / 8) = 20 bytes for both parameter sets, encoding both the \texttt{k} FORS leaf indices and the \texttt{h\_sl} hypertree path indices in a unified digest (see Section~10.5).
...
0x0A: FORS_PRF:     PRF for FORS secret key derivation
...
0x0A & \texttt{FORS\_PRF}       & tree\_idx & 0       & leaf\_idx \\
